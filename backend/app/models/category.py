...
class Category(BaseModel):
    ...
    __table_args__ = (
        UniqueConstraint(
            "user_id", "name", "transaction_type", name="uq_user_category_name_type"
        ),
        CheckConstraint("LENGTH(TRIM(name)) > 0", name="check_category_name_not_empty"),
    )
    ...
    name: Mapped[str] = mapped_column(String(100), unique=True, doc="Category name")
    transaction_type: Mapped[TransactionType] = mapped_column(
        Enum(TransactionType), doc="Operation type used category"
    )
