...
class Saving(BaseModel):
    ...
    __table_args__ = (
        ...
        CheckConstraint("LENGTH(TRIM(name)) > 0", name="check_saving_name_not_empty"),
    )
    user_id: Mapped[int] = mapped_column(
        ForeignKey("users.id"), doc="ID of the user who owns this savings goal"
    )
    name: Mapped[str] = mapped_column(String(100), doc="Name of the savings goal")
    final_amount: Mapped[int] = mapped_column(doc="Target amount to save")
    deadline: Mapped[date | None] = mapped_column(
        Date, nullable=True, doc="Optional target date to achieve the goal"
    )
    is_completed: Mapped[bool] = mapped_column(
        default=False, doc="Whether the goal has been achieved"
    )
