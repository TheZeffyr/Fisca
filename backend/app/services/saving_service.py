...
class SavingService:
    ...
    async def create(
        ...
        logger.info(f"Saving created successfully: id={saving.id}, user_tg_id={user_tg_id}, name={name}, final_amount={final_amount}")
    ...
    async def update_name(self, saving_id: int, name: str) -> SavingDTO:
        ...
        if saving is None:
            logger.warning(f"Saving not found when updating name: saving_id={saving_id}")
    ...
    async def update_deadline(self, saving_id: int, deadline: date) -> SavingDTO:
        ...
        if saving is None:
            logger.warning(f"Saving not found when updating deadline: saving_id={saving_id}")
            raise SavingNotFoundError()
        
        await self.session.commit()
        logger.info(f"Saving deadline updated: saving_id={saving_id}, new_deadline={deadline}")
        return SavingDTO.model_validate(saving)
    ...
    async def mark_completed(self, saving_id: int) -> SavingDTO:
        ...
        if saving is None:
            logger.warning(f"Saving not found when marking completed: saving_id={saving_id}")
            raise SavingNotFoundError()
        
        await self.session.commit()
        logger.info(f"Saving marked as completed: saving_id={saving_id}, name={saving.name}")
        return SavingDTO.model_validate(saving)
