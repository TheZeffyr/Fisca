...
class CategoryService:
    ...
    async def update(
        ...
        if existing_category is None:
            logger.warning(
                f"Category not found when updating: category_id={category_id}"
            )
        ...
        if name is not None and name != existing_category.name:
            name_exists = await self.repository.exists(
                user_id=existing_category.user_id, name=name
            )
            if name_exists:
                logger.warning(
        ...
        if (
            ...
            and transaction_type != existing_category.transaction_type
        ):
        ...
        updated_category = await self.repository.update(
            category_id=category_id, **update_data
        )

        if updated_category is None:
            logger.warning(
                f"Category not found during update: category_id={category_id}"
            )
            raise CategoryNotFoundError(f"Category with id {category_id} not found")
    ...
    async def delete(self, category_id: int) -> None:
        ...
        if not is_deleted:
            logger.warning(
                f"Category not found when deleting: category_id={category_id}"
            )
            raise CategoryNotFoundError(f"Category with id {category_id} not found")
